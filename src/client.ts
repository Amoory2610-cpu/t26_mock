...
document.getElementById('btnRoomSearch')!.addEventListener('click', async () => {
    ...
    const city: string = (document.getElementById('cityName') as HTMLInputElement).value;
    const hotel: string = (document.getElementById('hotelName') as HTMLInputElement).value;
    const date: string = (document.getElementById('checkInDate') as HTMLInputElement).value;
    ...
    const response = await fetch(`/search/${city}/${hotel}/${date}`);
    ...
    result.forEach((room: Room) => {
        document.getElementById('searchResults')!.innerHTML +=
            `<p>${room.hotel} in ${room.city}, ${room.date}, ${room.rtype}, ${room.nbeds} beds, £${room.price}
            <button type='button' class='btnBook' data-id='${room.id}'>Book</button></p>`;
    });
    ...
    document.querySelectorAll('.btnBook').forEach((btn) => {
        btn.addEventListener('click', async () => {
            const id = (btn as HTMLElement).dataset.id;
            await fetch(`/room/book/${id}`, { method: 'POST' });
        });
    });
...
document.getElementById('btnRoomAdd')!.addEventListener('click', async () => {
    ...
    await fetch('/room/new', {
        ...
        body: JSON.stringify({ city, hotel, date, rtype, nbeds, price: thePrice })
    });
